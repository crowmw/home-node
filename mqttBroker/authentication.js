...
exports.authenticate = function(client, username, password, callback) {
  axios
    .post('http://192.168.1.58:8080/signin', {
      email: username,
      password: password.toString()
    })
    .then(response => {
      console.log(
        `$(chalk.gray(moment().format('DD.MM.YYYY hh:mm:ss')) -  $(chalk.green('AUTHENTICATED'))  $(username)`
      );
    ...
    .catch(error => {
      console.log(
        chalk.gray(moment().format('DD.MM.YYYY hh:mm:ss') + ' - ') +
          chalk.red('AUTHENTICATION_FAILED') +
          '  ' +
          username +
          ' ' +
          password.toString()
